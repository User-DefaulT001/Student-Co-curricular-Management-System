...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'add' && $role === 'student') {
        ...
        $query = "INSERT INTO clubs (user_id, club_name, role, join_date, status, created_at) VALUES ('$user_id', '$club_name', '$role_held', '$join_date', '$status', NOW())";
...
if ($role === 'admin') {
    ...
    $table_query = "SELECT c.*, u.username FROM clubs c JOIN users u ON c.user_id = u.user_id ORDER BY c.join_date DESC";
} else {
...
<head>
    ...
    <style>
        ...
        .btn-circle { width: 30px; height: 30px; padding: 6px 0px; border-radius: 15px; text-align: center; font-size: 12px; line-height: 1.42857; }        .dataTables_length select { margin: 0 5px; height: auto !important; padding: 2px 5px !important; display: inline-block !important; }
                ...
                <div class="container-fluid">
                            ...
                            <div class="card border-left-primary shadow h-100 py-2">
                                <div class="card-body">
                                    <div class="row no-gutters align-items-center">
                                        <div class="col mr-2">
                                            <div class="text-xs font-weight-bold text-primary text-uppercase mb-1"><?php echo ($role === 'admin') ? "Global Club Records" : "Total Clubs Joined"; ?></div>
                    ...
                    <div class="card shadow mb-4">
                        <div class="card-header card-header-main py-3">
                            <h6 class="m-0 font-weight-bold text-white"><?php echo ($role === 'admin') ? "All Student Memberships" : "My Membership Record List"; ?></h6>
                        </div>
                        <div class="card-body">
                                ...
                                <table class="table table-hover" id="clubTable" width="100%" cellspacing="0">
                                    <thead>
                                        <tr>
                                            ...
                                            <th>Club Name</th><th>Role</th><th>Join Date</th><th>Status</th><th class="text-center">Action</th>
                                        </tr>
                                    </thead>
                                    <tbody>
                                        <?php
                                        ...
                                        <tr>
                                            <?php if($role === 'admin'): ?>
                                                <td><span class='badge badge-dark'><?php echo htmlspecialchars($row['username']); ?></span></td>
                                            <?php endif; ?>
                                            ...
                                            <td class='text-center'>
                                                <button class='btn btn-circle btn-sm btn-info edit-btn' data-id='<?php echo $row['club_id']; ?>' data-name='<?php echo htmlspecialchars($row['club_name']); ?>' data-role='<?php echo htmlspecialchars($row['role']); ?>' data-date='<?php echo $row['join_date']; ?>' data-status='<?php echo $row['status']; ?>' data-toggle='modal' data-target='#editClubModal'><i class='fas fa-edit'></i></button>
                                                <a href='?delete=<?php echo $row['club_id']; ?>' class='btn btn-circle btn-sm btn-danger' onclick='return confirm("Confirm delete?")'><i class='fas fa-trash'></i></a>
                                            </td>
                                        </tr>
                                        <?php endwhile; ?>
                                    </tbody>
    ...
    <div class="modal fade" id="addClubModal" tabindex="-1" role="dialog">
        <div class="modal-dialog" role="document">
            <form action="" method="POST" class="modal-content">
                ...
                <div class="modal-footer"><button type="submit" class="btn btn-primary">Add Record</button></div>
            </form>
        </div>
    </div>
